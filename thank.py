...
cursor.execute("CREATE table thank (id BIGINT PRIMARY KEY auto_increment,month VARCHAR(255),target TEXT,content TEXT NOT NULL);")
cursor.execute("CREATE table thankUrl (id BIGINT PRIMARY KEY auto_increment,month VARCHAR(255),url TEXT NOT NULL);")
...
def thankPDF(title, url):
  ...
  try:
    ...
    url = "https://www.vghtpe.gov.tw/vghtpe/files/PDF/thank/"+ urllib.parse.quote(needTranscode)
  ...
  for r in result:
    #感謝函月份、感謝對象、感謝內容
    cursor.execute("INSERT INTO thank (month, target, content) VALUES (%s, %s,%s)",(title,r[0], r[1]))
    con.commit()
  ...
  try:
    ...
    for tr in result:
      try:
        ...
        if target != "摘要":
          #感謝函月份、感謝對象、感謝內容
          cursor.execute("INSERT INTO thank (month, target, content) VALUES (%s, %s,%s)",(title, target, content))
          con.commit()
      except:
        try:
          td = tr.find_all("td")
...
for td in result:
  ...
  if len(item) != 0:
    ...
    # 每月感謝函名稱、URL
    cursor.execute("INSERT INTO thankUrl (month, url) VALUES (%s, %s)",(title, url))
    con.commit()
